...
def extract_token(params):
    ...
    if params.get("type") != "TOKEN":
        ...
        raise Exception('Expected "event.type" parameter to have value "TOKEN"')
    ...
    if len(parts) != 2 or parts[0].lower() != "bearer":
        ...
        logger.error(f'Invalid Authorization token - {token_string} does not match "Bearer .*"')
        raise Exception(f'Invalid Authorization token - {token_string} does not match "Bearer .*"')
